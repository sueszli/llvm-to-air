...
def _mandelbrot_numpy_numba_kernel(width: int, height: int, max_iter: int, x_min: float, x_max: float, y_min: float, y_max: float) -> np.ndarray:
    # super slow because we have to allocate temporary arrays for every intermediate step of the vectorization
...
@njit
def _mandelbrot_numba_list_kernel(width: int, height: int, max_iter: int, x_min: float, x_max: float, y_min: float, y_max: float):
    result = List()
    # Pre-allocate if possible? No, user asked for "without numpy" usually implies dynamic list or similar to plain.
    # But for performance with lists, append is what we do.
    # To hint types to Numba, we can append a dummy and clear, or just rely on flow.
    # numba.typed.List() is generic?
    # Actually, explicit typing helps.
    # But let's try just append.
...
def run_numba_list() -> list[float]:
    x_min, x_max = -2.5, 1.0
    y_min, y_max = -1.0, 1.0

    # We need to ensure List is typed as float64 usually?
    # Or just let it infer from append.
    result = _mandelbrot_numba_list_kernel(WIDTH, HEIGHT, MAX_ITER, x_min, x_max, y_min, y_max)
    return list(result)
...
if __name__ == "__main__":
    ...
    start_numba_list = time.perf_counter()
    result_numba_list = run_numba_list()
    end_numba_list = time.perf_counter()
    numba_list_time = end_numba_list - start_numba_list
