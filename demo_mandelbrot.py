...
if __name__ == "__main__":
    ...
    print(f"\nspeedups:")
    print(f"gpu vs numpy: {numpy_time/gpu_time:.2f}x")
    print(f"gpu vs numba: {numba_time/gpu_time:.2f}x")
    ...
    # print("gpu vs numpy vs numba vs cpu:")
    # for i in range(min(10, len(result_gpu))):
    #     print(f"  pixel {i}: gpu={int(result_gpu[i])}, numpy={int(result_numpy[i])}, numba={int(result_numba[i])}, cpu={int(result_cpu[i])}")
