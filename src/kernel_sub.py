...
def _gen_kernel_sub() -> ModuleOp:
    module = ModuleOp([])
    # void sub(float* A, float* B, float* C, i32 N, i32 global_id)
    args = [
    ...
    sub_func = func.FuncOp("sub", func_type)
    module.body.blocks[0].add_op(sub_func)

    entry_block = sub_func.body.blocks[0]
    ...
    # C[id] = A[id] - B[id]
    diff_val = builder.insert(arith.SubfOp(val_A, val_B)).results[0]
    ptr_C = builder.insert(llvm.GEPOp(arg_C, [llvm.GEP_USE_SSA_VAL], f32, ssa_indices=[id_i64])).results[0]
    builder.insert(llvm.StoreOp(diff_val, ptr_C))
...
@lru_cache(None)
def kernel_sub_binary():
    buf = StringIO()
    Printer(stream=buf).print_op(_gen_kernel_sub())
    ...
    air_llvm_text = to_air(trans_proc.stdout, kernel_overrides={"sub": {"4": "global_id"}})
