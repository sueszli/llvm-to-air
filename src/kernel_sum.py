...
def _gen_kernel_sum() -> ModuleOp:
    module = ModuleOp([])
    # void sum(float* A, float* B, i32 M, i32 N, i32 global_id)
    # A is input (MxN), B is output (Mx1 sums)
    ...
    sum_func = func.FuncOp("sum", func_type)
    module.body.blocks[0].add_op(sum_func)

    entry_block = sum_func.body.blocks[0]
    ...
    builder.insert(llvm.StoreOp(final_sum, ptr_B))
...
@lru_cache(None)
def kernel_sum_binary():
    buf = StringIO()
    Printer(stream=buf).print_op(_gen_kernel_sum())
    ...
    air_llvm_text = to_air(trans_proc.stdout, kernel_overrides={"sum": {"4": "global_id"}})
