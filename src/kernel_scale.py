...
def _gen_kernel_scale() -> ModuleOp:
    module = ModuleOp([])
    # void scale(float* A, float* C, float scalar, i32 N, i32 global_id)
    args = [
    ...
    scale_func = func.FuncOp("scale", func_type)
    module.body.blocks[0].add_op(scale_func)

    entry_block = scale_func.body.blocks[0]

    builder = Builder(InsertPoint.at_end(entry_block))

    arg_A, arg_C, arg_scalar, arg_N, arg_id = entry_block.args
    ...
    # C[id] = A[id] * scalar
    mul_val = builder.insert(arith.MulfOp(val_A, arg_scalar)).results[0]
    ptr_C = builder.insert(llvm.GEPOp(arg_C, [llvm.GEP_USE_SSA_VAL], f32, ssa_indices=[id_i64])).results[0]
    builder.insert(llvm.StoreOp(mul_val, ptr_C))
...
@lru_cache(None)
def kernel_scale_binary():
    buf = StringIO()
    Printer(stream=buf).print_op(_gen_kernel_scale())
    ...
    air_llvm_text = to_air(trans_proc.stdout, kernel_overrides={"scale": {"4": "global_id"}})
