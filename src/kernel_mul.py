...
def _gen_kernel_mul() -> ModuleOp:
    module = ModuleOp([])
    # void mul(float* A, float* B, float* C, i32 N, i32 global_id)
    args = [
    ...
    mul_func = func.FuncOp("mul", func_type)
    module.body.blocks[0].add_op(mul_func)

    entry_block = mul_func.body.blocks[0]
    ...
    # C[id] = A[id] * B[id]
    mul_val = builder.insert(arith.MulfOp(val_A, val_B)).results[0]
...
@lru_cache(None)
def kernel_mul_binary():
    buf = StringIO()
    Printer(stream=buf).print_op(_gen_kernel_mul())
    ...
    air_llvm_text = to_air(trans_proc.stdout, kernel_overrides={"mul": {"4": "global_id"}})
